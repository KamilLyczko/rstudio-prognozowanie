...
for(i in 1:length(d_time_series)) {
  train <- window(d_time_series[[i]], 
                  end = weekly_freq_day_number(length(d_time_series[[i]]) - 30))
  test <- window(d_time_series[[i]],
                 start = weekly_freq_day_number(length(d_time_series[[i]]) - 29))
  ...
  plots <- list(
    ...
    generate_forecast_plot2(predictions,
                            paste0("Wykres prognoz liczb śmierci dla ", i, " fali"),
                            "liczba śmierci") + set_titles_size(),
