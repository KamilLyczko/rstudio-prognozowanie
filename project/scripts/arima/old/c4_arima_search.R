...
train <- window(confirmed4, end = weekly_freq_day_number(length(confirmed4) - 30))
diff_list <- make_stationary_ts(train)
main_title <- 10
axis_titles <- 10
plots <- list()
plots[[1]] <- generate_ts_time_plot(train, "Wykres czasowy liczb zakażeń dla 4 fali", 
                                    "liczba zakażeń") + set_titles_size(main_title, axis_titles)
plots[[2]] <- generate_ts_time_plot(diff_list$diff_ts, 
                                    paste0("Wykres czasowy szeregu liczb zakażeń dla 4 fali po różnicowaniach (sezonowe: ",
                                           diff_list$diffs[1], ", pierwsze: ", diff_list$diffs[2], ")")) +
  set_titles_size(main_title, axis_titles)
plots[[3]] <- ggAcf(diff_list$diff_ts) +
  ggtitle("Wykres autokorelacji szeregu liczb zakażeń dla 4 fali po różnicowaniach") +
  set_titles_size(main_title, axis_titles)
plots[[4]] <- ggPacf(diff_list$diff_ts) +
  ggtitle("Wykres autokorelacji częściowej szeregu liczb zakażeń dla 4 fali po różnicowaniach") +
  set_titles_size(main_title, axis_titles)
...
#Na podstawie ACF: ARIMA(0,1,3)(0,1,1), ARIMA(0,1,6)(0,1,1)
#Na podstawie PACF: ARIMA(3,1,0)(2,1,0), ARIMA(3,1,0)(1,1,0)

#wybieranie najlepszych modeli spośród kandydatów
orders_cand <- list(c(0,1,3,0,1,1), c(0,1,2,0,1,1), c(0,1,6,0,1,1), c(0,1,3,1,1,1),
                    c(3,1,0,2,1,0), c(2,1,0,2,1,0), c(3,1,0,1,1,0), c(3,1,0,2,1,1))
...
test <- window(confirmed4, start = weekly_freq_day_number(length(confirmed4) - 29))
best_model <- best_models[[find_best_forecast_model(best_models, test)]]
#Najlepszy model:
summary(best_model)
#Najlepszy model: ARIMA(0,1,6)(0,1,1)[7]
...
plots <- list(
  generate_fit_plot(best_model,
                    paste0("Wykres dopasowania modelu ", 
                           ...
                           " do szeregu liczb zakażeń dla 4 fali"),
                    "liczba zakażeń"),
  generate_forecast_plot(forecast,
                         "Wykres prognoz liczb zakażeń dla 4 fali",
                         "liczba zakażeń"),
...
#value 1252.352 1280.915 3556322 1885.821 39.54462