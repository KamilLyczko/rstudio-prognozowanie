...
train <- window(deaths1, end = weekly_freq_day_number(length(deaths1) - 30))
diff_list <- make_stationary_ts(train)
main_title <- 10
axis_titles <- 10
plots <- list()
plots[[1]] <- generate_ts_time_plot(train, "Wykres czasowy liczb śmierci dla 1 fali", 
                                    "liczba śmierci") + set_titles_size(main_title, axis_titles)
plots[[2]] <- generate_ts_time_plot(diff_list$diff_ts, 
                                    paste0("Wykres czasowy szeregu liczb śmierci dla 1 fali po różnicowaniach (sezonowe: ",
                                           diff_list$diffs[1], ", pierwsze: ", diff_list$diffs[2], ")")) +
  set_titles_size(main_title, axis_titles)
plots[[3]] <- ggAcf(diff_list$diff_ts) +
  ggtitle("Wykres autokorelacji szeregu liczb śmierci dla 1 fali po różnicowaniach") +
  set_titles_size(main_title, axis_titles)
plots[[4]] <- ggPacf(diff_list$diff_ts) +
  ggtitle("Wykres autokorelacji częściowej szeregu liczb śmierci dla 1 fali po różnicowaniach") +
  set_titles_size(main_title, axis_titles)
grid.arrange(grobs = plots, ncol = 1)
#różnicowanie uzyskane przez make_stationary_ts: sezonowe - 0, pierwsze - 1

#Na podstawie ACF: ARIMA(0,1,2)
#Na podstawie PACF: ARIMA(2,1,0)

#wybieranie najlepszych modeli spośród kandydatów
orders_cand <- list(c(0,1,2), c(0,1,1), c(0,1,3), c(1,1,2), c(2,1,2),
                    c(2,1,0), c(1,1,0), c(3,1,0), c(2,1,1), c(1,1,1))
...
test <- window(deaths1, start = weekly_freq_day_number(length(deaths1) - 29))
best_model <- best_models[[find_best_forecast_model(best_models, test)]]
#Najlepszy model:
summary(best_model)
#Najlepszy model: ARIMA(2,1,2)
...
plots <- list(
  generate_fit_plot(best_model,
                    paste0("Wykres dopasowania modelu ", 
                           ...
                           " do szeregu liczb śmierci dla 1 fali"),
                    "liczba śmierci"),
  generate_forecast_plot(forecast,
                         "Wykres prognoz liczb śmierci dla 1 fali",
                         "liczba śmierci"),
...
time_plots[[1]] <- generate_ts_time_plot(add1_train, 
                                         "Wykres czasowy liczb śmierci zwiększonych o 1 dla 1 fali", 
                                         "liczba śmierci + 1") +
  set_titles_size(main_title, axis_titles)
time_plots[[2]] <- generate_ts_time_plot(boxcox_add1_train, 
                                         paste0("Wykres czasowy liczb śmierci dla 1 fali po transformacji Boxa-Coxa (lambda = ",
                                                signif(lambda, 3), ")"), 
                                         "BoxCox(liczba śmierci + 1)") +
  set_titles_size(main_title, axis_titles)
time_plots[[3]] <- generate_ts_time_plot(diff_list$diff_ts, 
                                         paste0("Wykres czasowy szeregu liczby śmierci dla 1 fali po ", 
                                                "transformacji Box-Cox i różnicowaniach (sezonowe: ",
                                                diff_list$diffs[1], ", pierwsze: ", diff_list$diffs[2], ")")) +
  set_titles_size(main_title, axis_titles)
cor_plots[[1]] <- ggAcf(diff_list$diff_ts) +
  ggtitle(paste0("Wykres autokorelacji transformowanego szeregu liczb śmierci dla 1 fali po różnicowaniach ",
                 "s: ", diff_list$diffs[1], ", p: ", diff_list$diffs[2], ")")) +
  set_titles_size(main_title, axis_titles)
cor_plots[[2]] <- ggPacf(diff_list$diff_ts) +
  ggtitle(paste0("Wykres autokorelacji częściowej transformowanego szeregu liczb śmierci dla 1 fali ", 
...
#Na podstawie ACF: ARIMA(0,1,2)(0,0,1)[7]
orders_cand <- list(c(0,1,2,0,0,1), c(0,1,1,0,0,1), c(1,1,0,0,0,1), c(2,1,0,0,0,1),
                    c(0,1,2,0,0,2), c(0,1,2,1,0,0), c(0,1,2,2,0,0), c(0,1,2,1,0,1))
...
#Najlepszy model: ARIMA(0,1,1)(1,0,1)[7]
...
plots <- list(
  generate_fit_plot(best_model2,
                    paste0("Wykres dopasowania modelu ", 
                           ...
                           " do zmodyfikowanego szeregu liczb śmierci dla 1 fali"),
                    "liczba śmierci + 1"),
