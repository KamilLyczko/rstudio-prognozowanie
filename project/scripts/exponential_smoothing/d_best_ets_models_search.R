...
mes <- "Modele szeregóW czasowych liczb śmierci:\n\n"
for(i in 1:length(models)){
  mes <- paste0(mes, i, " fala:\t", models[[i]]$method, "\n")
}
cat(mes)
...
for(i in 1:length(models)) {
  ...
  save_forecasts_to_csv(forecast, 
                        paste0("d", i, "_ets_forecasts.csv"), 
                        "exponential_smoothing/ets")
  save_df_to_csv(calculate_ex_post_errors(forecast, test),
                 paste0("d", i, "_ets_errors.csv"),
                 "exponential_smoothing/ets")
}

rm(i, models, train, test, h, forecast, mes)
