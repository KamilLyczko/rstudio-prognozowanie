...
for(i in 1:length(c_time_series)) {
  ...
  if(i != 4)
    show(generate_naive_forecasts_plot(train, confirmed_forecasts[[i]], plot_title1, "liczba zaka¿eñ"))
  else
  ...
  show(generate_naive_test_comparison_plot(test, confirmed_forecasts[[i]], plot_title2, "liczba zaka¿eñ"))
}
