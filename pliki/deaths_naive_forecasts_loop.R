...
for(i in 1:length(d_time_series)) {
  ...
  show(calculate_ex_post_errors(deaths_forecasts[[i]]$naive, test))
  cat("sezonowa metoda naiwna:\n")
  show(calculate_ex_post_errors(deaths_forecasts[[i]]$snaive, test))
  cat("przyrostowa metoda naiwna:\n")
  show(calculate_ex_post_errors(deaths_forecasts[[i]]$drift, test))
}
